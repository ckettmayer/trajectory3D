...
###  2D trajectory mean squared displacement analysis ###
...
for tau in range(Nplot):
    ...
    for i in range(N-tau):
        d[i]=(x[i+tau]-x[i])**2 + (y[i+tau]-y[i])**2  # Computes every squared displacement for a lag time tau
...
def anomalous_diff(lag,D_gen,alpha,s):
    return (4 * D_gen*lag**alpha+s)

p0_an=[0.1, 1, 0.1] # Initial parameters
...
def confined_diff(lag,Dm,L,s):
    return ((L**2/3)*(1-np.exp(-(12*Dm*lag)/L**2))) 

def directed_diff(lag,D,V,s):
    return (4 * D * lag + (V * lag)**2 + s)
...
plt.plot(lag_model, anomalous_diff(lag_model, *popt_an),
            'c-', label=r'MSD($\tau$)= MSD$_0$+4D$\tau^\alpha$')
